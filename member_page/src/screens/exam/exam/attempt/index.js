...
export default function ExamAttemptBox() {
  const {id} = useParams();
  ...
  const {data, loading, error} = useQuery(
    ...
    {
      ...API.DEFAULT_OPTIONS,
      variables: {token: AuthHelper.token()},
    },
  );
  ...
  if (loading) {
    return <Box title="Attempts" padding>Loading</Box>;
  }

  if (error) {
    return <Box title="Attempts" padding>{error.message}</Box>;
  }

  if (!data) {
    return <Box title="Attempts" padding>No Data</Box>;
  }

  const attempts = data.data.examAttempts.filter;
