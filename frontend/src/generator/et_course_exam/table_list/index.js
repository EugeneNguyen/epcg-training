...
import EtCourseExamTerTable from './excel_table';
import EtCourseExamTable from './table';

export default function TableEtCourseExamList({where, excludeColumns, relationshipName}) {
  ...
  let refetch = null;
  const Table = excelTable ? EtCourseExamTerTable : EtCourseExamTable;
  return (
    <div className="space-y-4">
    <Button onClick={() => setExcelTable(!excelTable)}>Switch</Button>
      <Box title="List EtCourseExam">
        <Table
          limit={limit}
          offset={offset}
          where={where}
