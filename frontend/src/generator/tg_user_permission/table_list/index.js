...
import TgUserPermissionTerTable from './excel_table';
import TgUserPermissionTable from './table';

export default function TableTgUserPermissionList({where, excludeColumns, relationshipName}) {
  ...
  let refetch = null;
  const Table = excelTable ? TgUserPermissionTerTable : TgUserPermissionTable;
  return (
    <div className="space-y-4">
    <Button onClick={() => setExcelTable(!excelTable)}>Switch</Button>
      <Box title="List TgUserPermission">
        <Table
          limit={limit}
          offset={offset}
          where={where}
